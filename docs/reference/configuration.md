...
| `AGENTIHOOKS_MCP_SUPERVISOR` | `auto` | Daemon supervisor: `systemd` where a user bus exists, `pidfile` otherwise. Force either to exercise both paths on one machine. Drive the daemon with `agentihooks mcp start\|stop\|restart\|status`. |
